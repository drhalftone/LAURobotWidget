...
LAURobotWidget::LAURobotWidget(QWidget *parent) : QWidget(parent), robot(NULL)
{
    ...
    robot = new LAURobotObject(QString());
    if (robot->isValid()) {
...
/****************************************************************************/
/****************************************************************************/
LAURobotObject::LAURobotObject(QString portString, QObject *parent) : QObject(parent), firmwareString(QString("DEMO"))
{
...
QByteArray LAURobotObject::appendCRC(QByteArray byteArray, CRC state)
{
    ...
    unsigned int crc = 0;
    for (int byte = 0; byte < byteArray.length(); byte++) {
        crc += (unsigned char)(byteArray.at(byte) & 0x7f);
    }
    // APPEND THE CRC CHECK SUM IN BIG ENDIAN FORMAT
    byteArray.append((char)(crc & 0x7F));
#endif
    return (byteArray);
}
